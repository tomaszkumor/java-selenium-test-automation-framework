...
public class SearchBarHotels extends SearchBarHotelsLocators {
    ...
    private String getExpectedCityValueAfterInput(PhpTravelsModel phpTravelsModel) {
        HotelDestinationModel hotelDestination = getExpectedHotelDestinationFromDataProvider(phpTravelsModel);

        return getHotelDestinationName(hotelDestination);
    }

    private void findAndSelectCity(PhpTravelsModel phpTravelsModel) {
        HotelDestinationModel expectedHotelDestination = getExpectedHotelDestinationFromDataProvider(phpTravelsModel);
        ...
        //TODO: opakuj to w metode (az do logu)
    ...
    private String getHotelDestinationName(HotelDestinationModel hotelDestination) {
    ...
    private void clickOnDateInput(CheckType checkType) {
        switch (checkType) {
            case IN -> {
                By locator = By.xpath("//input[@id = 'checkin']");
                check.isElementPresentByLocator(locator, 10);
                click.clickOnEnabledElement(checkInDateInput, 15);
            }
            case OUT -> {
                By locator = By.xpath("//input[@id = 'checkout']");
                check.isElementPresentByLocator(locator, 10);
                click.clickOnEnabledElement(checkOutDateInput, 15);
            }
        }
    ...
    private AccommodationModel getExpectedAccommodationFromDataProvider(PhpTravelsModel phpTravelsModel) {
        return phpTravelsModel.getHotelsPageModel().getAccommodation();
    }

    private List<ChildModel> getExpectedChildrenFromDataProvider(PhpTravelsModel phpTravelsModel) {
        return phpTravelsModel.getHotelsPageModel().getChildren();
    }

    private HotelDestinationModel getExpectedHotelDestinationFromDataProvider(PhpTravelsModel phpTravelsModel) {
        return phpTravelsModel.getHotelsPageModel().getHotelDestination();
    }

    private String getExpectedNationalityFromDataProvider(PhpTravelsModel phpTravelsModel) {
        return phpTravelsModel.getHotelsPageModel().getNationality();
    }
